...
class CreacionEncuestaController extends ChangeNotifier {
  final _repository = RepositoryImpl();
  final subject = 'Atividade - Creación de la encuesta';
  final doc = 'dos/creacion-encuesta/atividade/';
  final task = 'creacionEncuestaCompleted';
  final studentGroup = [];
  ...
  Map<String, String> setJson(List<dynamic> filePath) {
    final json = {'resposta_1_file': filePath[0].toString()};
    return json;
  }
  ...
  List<FileAttachment> createAttachments() {
    ...
    final attachment = [
      FileAttachment(firstArchive, fileName: 'Proposta La Encuesta'),
    ];
    ...
    try {
      for (final item in filePaths) {
        ...
        final snapshot = await firebaseStorage
            .ref()
            .child('dos-creacion-encuesta-arquivos/$email-img-$counter.jpeg')
            .putFile(file)
            .whenComplete(() => null);
...
Atividade - Creación de la encuesta, tarefa concluída!\n
$studentsNames\n
